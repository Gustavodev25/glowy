...
export async function GET(request: NextRequest) {
  try {
    ...
    return NextResponse.json({ error: 'Erro ao listar métodos' }, { status: 500 });
  }
}
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!luhnCheck(digits)) { if (!(isSandbox && ASAAS_TEST_CARDS.has(digits))) { return NextResponse.json({ error: 'Número do cartão inválido' }, { status: 400 }); } }
    ...
    if (!(expMonth >= 1 && expMonth <= 12)) {
      return NextResponse.json({ error: 'Mês de expiração inválido' }, { status: 400 });
    }
    const now = new Date();
    const expYear = expYearRaw < 100 ? 2000 + expYearRaw : expYearRaw;
    if (expYear < now.getFullYear() || expYear > now.getFullYear() + 25) {
      return NextResponse.json({ error: 'Ano de expiração inválido' }, { status: 400 });
    }
    ...
    if (brand === 'American Express') {
      if (ccvDigits.length !== 4) return NextResponse.json({ error: 'CVV inválido' }, { status: 400 });
    } else {
      if (ccvDigits.length !== 3) return NextResponse.json({ error: 'CVV inválido' }, { status: 400 });
    }
    ...
    const methods = await prisma.paymentMethod.findMany({ where: { userId: user.userId }, orderBy: [{ isDefault: 'desc' }, { createdAt: 'desc' }] });
    return NextResponse.json({ success: true, methods: methods.map(m => ({ id: m.id, type: m.type, brand: m.brand, last4: m.last4, expMonth: m.expMonth, expYear: m.expYear, isDefault: m.isDefault })) });
  } catch (e) {
