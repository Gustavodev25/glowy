...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!businessHour) {
      console.log(
        `ℹ️ BusinessHours não encontrado para dia ${diaDaSemana}, usando horário padrão 8h-20h`,
      );
      businessHour = {
        openTime: "08:00",
        closeTime: "20:00",
    ...
    // Se o estabelecimento está fechado neste dia ou não tem horário definido
    ...
    const [closeHour, closeMinute] = businessHour.closeTime
      .split(":")
      .map(Number);
    ...
    if (businessHour.breakStart && businessHour.breakEnd) {
      const [breakStartHour, breakStartMinute] = businessHour.breakStart
        .split(":")
        .map(Number);
      const [breakEndHour, breakEndMinute] = businessHour.breakEnd
        .split(":")
        .map(Number);
    ...
    for (
      let minutos = openTimeMinutes;
      minutos + duracaoMinutos <= closeTimeMinutes;
      ...
      const horarioFim = new Date(
        horarioInicio.getTime() + duracaoMinutos * 60000,
      );
      ...
      for (const agendamento of agendamentos) {
        const agendamentoFim = new Date(
          agendamento.dataHora.getTime() + agendamento.duracao * 60000,
        );

        // Verifica se há sobreposição
        const hasOverlap =
          (horarioInicio >= agendamento.dataHora &&
            horarioInicio < agendamentoFim) ||
          (horarioFim > agendamento.dataHora && horarioFim <= agendamentoFim) ||
          (horarioInicio <= agendamento.dataHora &&
            horarioFim >= agendamentoFim);
